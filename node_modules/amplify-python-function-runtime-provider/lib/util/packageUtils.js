...
async function pythonPackage(context, params) {
    if (!params.lastPackageTimeStamp || params.lastBuildTimeStamp > params.lastPackageTimeStamp || params.currentHash) {
        ...
        if (params.service) {
            ...
            const pyVersion = await pyUtils_1.execAsStringPromise(`${pyBinary} --version`);
            const layerPythonPath = path.join(params.srcRoot, 'lib', 'python' + pyUtils_1.majMinPyVersion(pyVersion), 'site-packages');
            const pipEnvDir = await pyUtils_1.getPipenvDir(params.srcRoot);
            ...
            zipEntries.push({
                ...
                packageFolder: await pyUtils_1.getPipenvDir(params.srcRoot),
