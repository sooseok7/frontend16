...
async function getEnvironmentNetworkInfo(context, params) {
    ...
    if (subnetsCount > AvailabilityZones.length) {
        ...
        throw new Error(`The requested number of subnets exceeds the number of AZs for the region. ${amplify_cli_core_1.JSONUtilities.stringify({ subnets, azs: AZs })}`);
    }
    ...
    if (vpcId && !vpc.CidrBlock.endsWith(vpcMask)) {
        throw new Error('Not the right mask');
    }
