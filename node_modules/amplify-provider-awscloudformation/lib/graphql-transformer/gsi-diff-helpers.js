...
const getGSIDiffs = (current, next) => {
    ...
    return exports.generateGSIChangeList(currentIndexes, nextIndexes);
};
...
const generateGSIChangeList = (currentIndexes, nextIndexes) => {
    ...
    const modifiedIndexes = possiblyModifiedIndexNames
        .filter(indexName => exports.isIndexModified(currentIndexByIndexName[indexName], nextIndexByIndexName[indexName]))
        .map(indexName => ({
...
const isIndexModified = (currentIndex, nextIndex) => {
    const diffs = deep_diff_1.diff(currentIndex, nextIndex);
