...
const configurePermissionsBoundaryForExistingEnv = async (context) => {
    amplify_cli_core_1.setPermissionsBoundaryArn(await permissionsBoundarySupplier(context));
...
const configurePermissionsBoundaryForInit = async (context) => {
    var _a;
    const envName = context.exeInfo.localEnvInfo.envName;
    if ((_a = context === null || context === void 0 ? void 0 : context.exeInfo) === null || _a === void 0 ? void 0 : _a.isNewProject) {
        amplify_cli_core_1.setPermissionsBoundaryArn(await permissionsBoundarySupplier(context, { doPrompt: false, envNameSupplier: () => envName }), envName, context.exeInfo.teamProviderInfo);
    }
    else {
...
const permissionsBoundarySupplier = async (context, options) => {
    ...
    const defaultValue = amplify_cli_core_1.getPermissionsBoundaryArn(envName);
    ...
    const { permissionsBoundaryArn } = await inquirer_1.prompt({
        type: 'input',
...
const rolloverPermissionsBoundaryToNewEnvironment = async (context) => {
    ...
    if (typeof headlessPermissionsBoundary === 'string') {
        amplify_cli_core_1.setPermissionsBoundaryArn(headlessPermissionsBoundary, newEnv, context.exeInfo.teamProviderInfo);
        return;
    }
    const currBoundary = amplify_cli_core_1.getPermissionsBoundaryArn();
    if (!currBoundary) {
        return;
    }
    ...
    if (await isPolicyAccessible(context, currBoundary)) {
        amplify_cli_core_1.setPermissionsBoundaryArn(currBoundary, newEnv, context.exeInfo.teamProviderInfo);
    ...
    amplify_cli_core_1.setPermissionsBoundaryArn(await permissionsBoundarySupplier(context, { required: true, envNameSupplier: () => newEnv }), newEnv, context.exeInfo.teamProviderInfo);
};
