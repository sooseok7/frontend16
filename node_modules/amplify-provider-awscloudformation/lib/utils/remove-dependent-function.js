...
async function ensureValidFunctionModelDependencies(context, apiResource, allResources) {
    var _a, _b, _c;
        ...
        if (dependentFunctionResource.length === 0) {
            ...
            if (continueForcePush) {
                ...
                if (!continueToPush &&
                    ...
                    throw new Error(`In order to successfully deploy. Run “amplify update function” on the affected functions ${dependentFunctionsNames} and remove the access permission to ${modelsDeleted}.`);
                }
            }
            return dependentFunctionResource;
        }
...
async function getModelNameDiff(currentBackendDir, backendDir, apiResourceName) {
...
async function getModelNames(backendDir, apiResourceName) {
    const appsyncTableSuffix = '@model(appsync)';
    const resourceDirPath = path_1.default.join(backendDir, 'api', apiResourceName);
    const project = await graphql_transformer_core_1.readProjectConfiguration(resourceDirPath);
    const directivesMap = graphql_transformer_core_1.collectDirectivesByTypeNames(project.schema);
