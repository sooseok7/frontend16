...
const logger = aws_logger_1.fileLogger('CognitoUserPoolService');
...
class CognitoUserPoolService {
    ...
    async listUserPools() {
        if (this.cachedUserPoolIds.length === 0) {
            const result = await paged_call_1.pagedAWSCall(async (params, nextToken) => {
    ...
    async listUserPoolClients(userPoolId) {
        const userPoolClients = await paged_call_1.pagedAWSCall(async (params, nextToken) => {
    ...
    async listUserPoolIdentityProviders(userPoolId) {
        const identityProviders = await paged_call_1.pagedAWSCall(async (params, nextToken) => {
