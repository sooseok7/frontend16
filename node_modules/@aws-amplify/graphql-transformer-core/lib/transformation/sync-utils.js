...
function createSyncTable(context) {
    const stack = context.stackManager.getStackFor(graphql_transformer_common_1.SyncResourceIDs.syncTableName);
    const tableName = context.resourceHelper.generateResourceName(graphql_transformer_common_1.SyncResourceIDs.syncTableName);
    new aws_dynamodb_1.Table(stack, graphql_transformer_common_1.SyncResourceIDs.syncTableName, {
        tableName,
        partitionKey: {
...
function createSyncIAMRole(context, stack, tableName) {
    const role = new iam.Role(stack, graphql_transformer_common_1.SyncResourceIDs.syncIAMRoleName, {
...
function createSyncLambdaIAMPolicy(stack, name, region) {
...
function syncLambdaArnResource(name, region) {
    ...
    if (referencesEnv(name)) {
        Object.assign(substitutions, {
            env: cdk.Fn.ref(graphql_transformer_common_1.ResourceConstants.PARAMETERS.Env),
        });
    }
