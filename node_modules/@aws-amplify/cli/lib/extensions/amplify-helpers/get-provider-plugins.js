...
const executeProviderCommand = async (context, command, args = []) => {
    const providers = await Promise.all(Object.values(exports.getConfiguredProviders(context)).map(providerPath => Promise.resolve().then(() => __importStar(require(providerPath)))));
