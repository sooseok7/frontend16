...
exports.raiseEvent = exports.raiseIntenralOnlyPostEnvRemoveEvent = exports.isContainersEnabled = exports.executeCommand = void 0;
...
async function raisePreEvent(context) {
    await amplify_cli_core_1.executeHooks(amplify_cli_core_1.HooksMeta.getInstance(context.input, 'pre'));
...
async function raisePostEvent(context) {
    const { command, plugin } = context.input;
    if (!plugin || !EVENT_EMITTING_PLUGINS.has(plugin)) {
        await amplify_cli_core_1.executeHooks(amplify_cli_core_1.HooksMeta.getInstance(context.input, 'post'));
        return;
    }
    switch (command) {
        case 'init':
...
async function raiseIntenralOnlyPostEnvRemoveEvent(context, envName) {
    await raiseEvent(context, new amplify_event_1.AmplifyEventArgs(amplify_event_1.AmplifyEvent.InternalOnlyPostEnvRemove, new amplify_event_1.AmplifyInternalOnlyPostEnvRemoveEventData(envName)));
}
