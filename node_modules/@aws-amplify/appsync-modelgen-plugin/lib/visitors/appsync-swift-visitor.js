...
class AppSyncSwiftVisitor extends appsync_visitor_1.AppSyncModelVisitor {
    ...
    generateKeyRules(model) {
        ...
        if (this.config.usePipelinedTransformer) {
            let fieldDirectiveList = new Array();
                ...
                switch (directiveObj.directive.name) {
                    ...
                    case 'primaryKey':
                        ...
                        if (!directiveObj.directive.arguments.sortKeyFields) {
                            directiveObj.directive.arguments.sortKeyFields = new Array();
                        }
                        directiveObj.directive.arguments.sortKeyFields = [directiveObj.fieldName, ...directiveObj.directive.arguments.sortKeyFields];
                        const fields = directiveObj.directive.arguments.sortKeyFields.map((field) => `"${field}"`).join(', ');
