...
class Compiler {
    ...
    addTypeUsed(type) {
        ...
        if (graphql_1.isInterfaceType(type) || graphql_1.isUnionType(type)) {
            for (const concreteType of this.schema.getPossibleTypes(type)) {
                this.addTypeUsed(graphql_1.getNamedType(concreteType));
            }
        }
        ...
        if (graphql_1.isObjectType(type)) {
            for (const fieldType of type.getInterfaces()) {
                this.addTypeUsed(graphql_1.getNamedType(fieldType));
            }
            for (const field of Object.values(type.getFields())) {
                this.addTypeUsed(graphql_1.getNamedType(field.type));
            }
    ...
    compileOperation(operationDefinition) {
        ...
        this.addTypeUsed(graphql_1.getNamedType(selectionSet.selections[0].type));
        return {
            filePath,
            operationName,
            operationType,
