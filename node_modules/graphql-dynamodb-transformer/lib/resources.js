...
class ResourceFactory {
    ...
    makeUpdateResolver({ type, nameOverride, syncConfig, mutationTypeName = 'Mutation', timestamps, optionalNonNullableFields, }) {
        ...
        const isSyncEnabled = syncConfig ? true : false;
        const optionalNonNullableExpression = optionalNonNullableFields.map(graphql_mapping_template_1.str);
        return new cloudform_types_1.AppSync.Resolver({
            ApiId: cloudform_types_1.Fn.GetAtt(graphql_transformer_common_1.ResourceConstants.RESOURCES.GraphQLAPILogicalID, 'ApiId'),
            DataSourceName: cloudform_types_1.Fn.GetAtt(graphql_transformer_common_1.ModelResourceIDs.ModelTableDataSourceID(type), 'Name'),
            FieldName: fieldName,
            ...
            RequestMappingTemplate: graphql_mapping_template_1.print(graphql_mapping_template_1.compoundExpression([
                graphql_mapping_template_1.set(graphql_mapping_template_1.ref('optionalNonNullableFields'), graphql_mapping_template_1.list(optionalNonNullableExpression)),
                graphql_mapping_template_1.forEach(graphql_mapping_template_1.ref('field'), graphql_mapping_template_1.ref('optionalNonNullableFields'), [
                    graphql_mapping_template_1.iff(graphql_mapping_template_1.and([graphql_mapping_template_1.ref('context.arguments.input.keySet().contains($field)'), graphql_mapping_template_1.ref('util.isNull($context.args.input.get($field))')]), graphql_mapping_template_1.ref('util.error("An argument you marked as Non-Null is set to Null in the query or the body of your request.")')),
                ]),
    ...
    makeListResolver(type, improvePluralization, nameOverride, isSyncEnabled = false, queryTypeName = 'Query') {
        const fieldName = nameOverride ? nameOverride : graphql_transformer_common_1.graphqlName('list' + graphql_transformer_common_1.plurality(graphql_transformer_common_1.toUpper(type), improvePluralization));
