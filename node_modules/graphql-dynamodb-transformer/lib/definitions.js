...
function getFieldsOptionalNonNullableField(fields, obj) {
    const fieldMap = fields.reduce((map, field) => {
        map.set(field.name.value, field);
        return map;
    }, new Map());
    return obj.fields
        .filter(r => fieldMap.has(r.name.value) &&
        graphql_transformer_common_1.isNonNullType(r.type) &&
        fieldMap.get(r.name.value).type.kind !== graphql_1.Kind.NON_NULL_TYPE)
        .map(r => r.name.value);
}
exports.getFieldsOptionalNonNullableField = getFieldsOptionalNonNullableField;
