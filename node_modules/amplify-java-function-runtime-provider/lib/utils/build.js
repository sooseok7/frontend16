...
const buildResource = async (request) => {
    const resourceDir = path_1.join(request.srcRoot);
    const projectPath = path_1.join(resourceDir);
...
const installDependencies = (resourceDir) => {
    ...
    if (!fs_extra_1.default.existsSync(path_1.join(packageLibDir, constants_1.relativeShimJarPath))) {
        runPackageManager(path_1.join(packageLibDir, constants_1.relativeShimSrcPath), 'jar');
    }
};
