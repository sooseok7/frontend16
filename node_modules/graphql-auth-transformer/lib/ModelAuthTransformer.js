    ...
    constructor(config) {
      ...
      `);
        ...
        this.object = (def, directive, ctx) => {
            ...
            const modelConfiguration = new ModelDirectiveConfiguration_1.ModelDirectiveConfiguration(modelDirective, def, ctx.featureFlags.getBoolean('improvePluralization'));
...
Static group authorization should perform as expected.`);
            ...
            if (modelDirective) {
                ...
                const modelConfiguration = new ModelDirectiveConfiguration_1.ModelDirectiveConfiguration(modelDirective, parent, ctx.featureFlags.getBoolean('improvePluralization'));
...
operations will be generated by the CLI.`);
    ...
    isSubscriptionOwnerArgumentOptional(rules) {
        const ownerRules = rules.filter(rule => rule.allow === constants_1.OWNER_AUTH_STRATEGY);
        if (ownerRules.length > 1) {
            return true;
        }
        return rules.find(rule => (rule.allow === constants_1.GROUPS_AUTH_STRATEGY && !rule.groupsField) || rule.allow === 'private' || rule.allow === 'public')
            ? true
            : false;
    }
