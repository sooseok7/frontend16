...
const importS3 = async (context, serviceSelection, previousResourceParameters, providerPluginInstance, printSuccessMessage = true) => {
    let resourceName = s3_walkthrough_1.resourceAlreadyExists();
    ...
    const providerPlugin = providerPluginInstance || (await Promise.resolve().then(() => __importStar(require(serviceSelection.provider))));
...
const ensureAuth = async (context) => {
    while (!s3_walkthrough_1.checkIfAuthExists()) {
