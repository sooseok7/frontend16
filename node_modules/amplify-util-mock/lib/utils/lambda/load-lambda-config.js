...
const loadLambdaConfig = async (context, resourceName, overrideApiToLocal = false) => {
    ...
    const cfnParams = populate_cfn_params_1.populateCfnParams(context.print, resourceName, overrideApiToLocal);
    const processedLambda = lambda_1.lambdaFunctionHandler(lambdaDef[0], lambdaDef[1], {
        conditions: CFN_DEFAULT_CONDITIONS,
        params: cfnParams,
        exports: {},
        resources: {},
    });
    await populate_lambda_mock_env_vars_1.populateLambdaMockEnvVars(context, processedLambda);
    return processedLambda;
};
exports.loadLambdaConfig = loadLambdaConfig;
const isApiRunning = async () => {
    const result = await detect_port_1.default(api_1.MOCK_API_PORT);
