...
async function start(context) {
    ...
    if (!resourceName) {
        const choices = lodash_1.default.keys(lodash_1.default.get(ampMeta, ['function'])).filter(resourceName => amplify_category_function_1.isMockable(context, resourceName).isMockable);
        if (choices.length < 1) {
    ...
    const lambdaConfig = await load_lambda_config_1.loadLambdaConfig(context, resourceName);
    ...
    await amplify_category_function_1.getBuilder(context, resourceName, amplify_function_plugin_interface_1.BuildType.DEV)();
    const invoker = await amplify_category_function_1.getInvoker(context, { resourceName, handler: lambdaConfig.handler, envVars: lambdaConfig.environment });
    context.print.blue('Starting execution...');
    try {
        const result = await exports.timeConstrainedInvoker(invoker({ event }), context.input.options);
