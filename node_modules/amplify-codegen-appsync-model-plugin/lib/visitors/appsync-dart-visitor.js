...
class AppSyncModelDartVisitor extends appsync_visitor_1.AppSyncModelVisitor {
    ...
    generateConstructor(model, declarationBlock) {
        const args = `{${model.fields.map(f => `${f.isNullable ? '' : '@required '}this.${this.getFieldName(f)}`).join(', ')}}`;
        ...
        const factoryParam = `{${model.fields.map(f => `${f.isNullable ? '' : '@required '}${this.getNativeType(f)} ${this.getFieldName(f)}`).join(', ')}}`;
    ...
    generateCopyWithMethod(model, declarationBlock) {
        const copyParam = `{${model.fields.map(f => `${f.isNullable ? '' : '@required '}${this.getNativeType(f)} ${this.getFieldName(f)}`).join(', ')}}`;
